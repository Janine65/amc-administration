...
wxAMC.registeredModules.push("Tasks");


wxAMC.moduleClasses.Tasks = class {
  ...
  getUIConfig() {
    ...
    return {
      winLabel : "Tasks", winIcon : "webix_icon mdi mdi-calendar-check",
            ...
            { view : "toolbar",
              cols : [
                ...
                { id : "moduleTasks-deleteButton", view : "button",
                  ...
                  icon : "webix_icon mdi mdi-delete", click : () => { wxAMC.deleteHandler("Tasks"); }
                },
                { },
                { view : "button", label : "Save", width : "80",
                  ...
                  click : function() {
                    wxAMC.saveHandler("Tasks", [ "moduleTasks-detailsForm" ]);
                  }
                },
                { width : 6 }
              ]
  ...
  refreshData() {
    ...
    // create it now.
    const dataItems = wxAMC.getModuleData("Tasks");
    ...
    // for each task, we pull out the category and add a adresse to an object for
    ...
    const itemsAsArray = wxAMC.objectAsArray(tasksData);

    // Sort the array by the value property (ascending) so they appear in
    // alphabetical order.
    wxAMC.sortArray(itemsAsArray, "value", "A");
